...
class WeatherProvider {
  ...
  static Future<WeatherModel> FetchRawWeatherResponseOnCity(
    ...
    final response = await http.get(
        "https://api.openweathermap.org/data/2.5/forecast?q=$cityName&lang=ar&units=metric&appid=${apiKey}");
  ...
  static Future<WeatherModel>  FetchRawWeatherResponseOnLocation(
    ...
    final response = await http.get(
        "https://api.openweathermap.org/data/2.5/forecast?lat=${position.latitude}&lon=${position.longitude}&lang=ar&units=metric&appid=${apiKey}");
    if (response.statusCode == 200) {
      print(response.body);
      return  WeatherModel.fromJson(jsonDecode(response.body));
      print(response.body);
